...
string ObjectAnalysis::match_shape(InformationOfRegionFound inf)
{
    ...
    cout<<"Distance phi1:" << pow(fabs(objectModels[closest].median_ph1 - inf.ph1),2)<< " " << objectModels[closest].variance_ph1*4<<endl;
    cout <<"Distance phi2:" << pow(fabs(objectModels[closest].median_ph2 - inf.ph2),2) << " " << objectModels[closest].variance_ph2*4<<endl;

    if (pow(fabs(objectModels[closest].median_ph1 - inf.ph1),2) <= objectModels[closest].variance_ph1*4 || pow(fabs(objectModels[closest].median_ph2 - inf.ph2),2) <= objectModels[closest].variance_ph2*4)
...
void ObjectAnalysis::draw_moments(InformationOfRegionFound inf, int hyp)
{
    ...
    line(color_image, Point(inf.cx - adj, inf.cy - opp), Point(inf.cx + adj, inf.cy + opp), Scalar(0, 255, 0),2);
}
